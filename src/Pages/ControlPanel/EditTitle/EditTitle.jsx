...
const EditTitle = ({item, categories}) => {
    ...
    const onSubmit = async ({id, name, category, rating, year, isTrending, thumbnail}, {setSubmitting, setErrors}) =>{
        try {
            const credentialUser = await updateTitle({id, name, category, rating, year, isTrending, thumbnail});
            saveChangesMessage()
        } catch (error) {
            console.log(Error)
        }
                ...
                >
                    {({values, setFieldValue, handleSubmit, handleChange, errors, touched, resetForm, handleBlur, dirty}) => (
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="edit-title-grid">
                                ...
                                <div>
                                    ...
                                    <input 
                                            ...
                                            onChange={handleChange}
                                            onClick={() => setFieldValue('isTrending', !values.isTrending)}
                                            onBlur={handleBlur}
                                            name='trending'
                                        />
                                </div>
                                <div>
                                    {/* <button 
                                        ...
                                        onClick={() => console.log("Change thumbnail...")}
                                    >
                                        Change thumbnail...
                                    </button> */}
                                </div>
                                ...
                                <div className='mb-10 mt-4'>
                                    <input type='file' />
                                    <img className="object-contain h-36 w-80" src={item.thumbnail.regular.small}/>
                                </div>
                            <button 
                                type='submit' 
                                disabled={!dirty} 
                            ...
                            {
                                messageVisible 
                                    ? <div className='title-updated-message'>Title updated successfully!</div>
                                    : null
                            }
                            <button type='button' className="control-panel-button absolute top-6 right-8" onClick={() => addNewTitle()}>New title</button>
                        </form>
                    )}
                </Formik>
            </div>
